...
import pandas as pd
params = pd.DataFrame(columns = ['time_stamp','embed_dim','mem_dim','hidden_dim','optim','lr','w_d','x','type_parse'])
...
with open(os.path.join("best_model.txt"),"r") as mdf:
    for lines in mdf:
        ...
        name = ln.split(",")
        file = "_".join(name[:-3])
        params.loc[len(params)] = name        
        #for f in file:
            
        #print(name)
        #output_file = open(os.path.join("dst",file+"_target_pred_.pkl" ),"wb")
        #pickle.dump(example_dict, output_file)
        #print(output_file)
        break
