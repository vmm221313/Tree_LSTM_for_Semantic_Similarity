...
if (args.evaluate):
    ...
    with torch.no_grad():
        ...
        for idx in tqdm(range(len(test_dataset)), desc='Testing epoch  ' + str(self.epoch) + ''):
    ...
    with open(os.path.join(chkptloc,"result_"+args.model_type+".csv"),"a") as f:
        ...
        res=res+","+str(round(save_test_pearson,4))+","+str(round(save_test_spear,4))\
            +","+str(round(save_test_mse,4))+"\n"
        f.writelines(res)
