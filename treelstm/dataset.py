...
class SICKDataset(data.Dataset):
    def __init__(self, path, vocab, rel_vocab, num_classes):
        ...
        self.ltrees = self.read_trees(os.path.join(path, 'a.parents'), os.path.join(path, 'a.rels')) #CHANGED
        self.rtrees = self.read_trees(os.path.join(path, 'b.parents'), os.path.join(path, 'b.rels')) #CHANGED 
    ...
    def read_trees(self, token_filename, rel_filename): #CHANGED
        with open(token_filename, 'r') as f, open(rel_filename, 'r') as g:
            trees = [self.read_tree(token,tag) for (token, tag) in tqdm(zip(f.readlines(), g.readlines()))]
        return trees
